...
class NeuralNetMLP(object):
    ...
    def __init__(self, n_output, n_features, n_hidden=30,
        ...
        self.w1, self.w2 = self._initalize_weights()
    ...
    def _initalize_weights(self):
        size1 = self.n_hidden * (self.n_features + 1)
        w1 = np.random.uniform(-1.0, 1.0,
                               size=size1)
        w1 = w1.reshape(self.n_hidden, self.n_features + 1)
        size2 = self.n_output * (self.n_hidden + 1)
    ...
    def _L1_reg(self, lambda_, w1, w2):
        return (lambda_/2.0) * (np.abs(w1[:, 1:] ** 2).sum() + np.abs(w1[:, 1:] ** 2).sum())
    ...
    def fit(self, X, y, print_progress=False):
        ...
        for i in range(self.epochs):
            self.eta /= (1 + self.decrease_count * i)
            if print_progress:
...
if __name__ == '__main__':
    ...
    cost_avg = [np.mean(cost_ary[i]) for i in batches]
